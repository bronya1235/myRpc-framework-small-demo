	...
	public URI lookupService(String serviceName) throws IOException {
		...
		try(RandomAccessFile raf = new RandomAccessFile(file, "rw");
			...
			try {
				...
				metadata = bytes.length == 0? new Metadata(): SerializeUtil.parse(bytes);
		...
		try(RandomAccessFile raf = new RandomAccessFile(file, "rw");
			...
			try {
				...
				if(fileLength > 0) {
					...
					metadata = SerializeUtil.parse(bytes);
				} else {
				...
				bytes = SerializeUtil.serialize(metadata);
